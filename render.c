...
bool fractal_set(t_data *data, complex c)
{
	int     p;
	complex z;
...
int key_event(int keycode, t_data *data)
{
	...
	if(keycode == 1 || keycode == 125)
		data->m.b = data->m.b - 0.05 * data->m.scale;
	if(keycode == 69)
		data->m.scale = data->m.scale * 0.95;// *  pow(10, ft_brake(data));
	if(keycode == 78)
		data->m.scale = data->m.scale * 1.05;
	return(0);
}
...
int render(t_data *data)
{
	...
	while(y < data->h)
	{
		...
		while(x < data->w)
		{
			if(fractal_set(data,c) == 1 && (c.b > -2 && c.a < 2))
